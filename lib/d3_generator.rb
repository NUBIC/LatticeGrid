...
def d3_master_investigator_graph(investigator)
  ...
  add_investigator_to_graph(investigator, coauthor_ids, [])
end
...
def d3_investigator_imports(investigator, coauthor_ids, primary_name)
  ...
  the_arry << primary_name unless investigator.last_name.eql?(primary_name)
  investigator.colleague_coauthors.each do |colleague|
    next unless colleague
    the_arry << colleague.last_name if coauthor_ids.include?(colleague.id)
  end
  the_arry
end
...
def d3_investigator_edge_imports(investigator)
  ...
  investigator.colleague_coauthors.each do |colleague|
    ...
    first = first.delete(')')
    first = first.delete('.')
    the_arry << "RHLCCC.#{org_unit}.#{first}-" + colleague.last_name.delete("\'")
  end
