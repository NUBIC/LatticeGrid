...
namespace :awards do
  task :importData => :environment do
    ...
    puts "count of all proposals is #{@all_awards.length}" if LatticeGridHelper.verbose?
  ...
  task :cleanDates => :getAwards do
    block_timing("awards:cleanDates") do
  ...
  task :cleanPIassignments => :getAwards do
    block_timing("awards:cleanPIassignments") do
      no_pi_cnt = 0
      multiple_pi_cnt = 0
      corrected_pi_cnt = 0
      @all_awards.each do |award|
        pis = award.investigator_proposals.pis
        no_pi_cnt += 1 if pis.length == 0
        if pis.length > 1
          multiple_pi_cnt += 1
          main_pi = 0
          ...
          elsif main_pi > 1 && !has_pi_employee_id
