class Tag < ActiveRecord::Base
  ...
  class << self
    ...
    #  :count -  count is still the number of tags for tag_id and taggable_count is the number of distinct taggable_ids per tag_id
    ...
    def options_for_counts(options = {})
      options.assert_valid_keys(:start_at, :end_at, :count, :conditions, :at_least, :at_most, :order, :limit, :joins)
      options = options.dup
      ...
      select << " , count(distinct #{Tagging.table_name}.taggable_id) AS taggable_count" if options[:count] 
