...
class ProfilesController < ApplicationController
  ...
  def list_summaries_by_program
    if is_admin?
      ...
      respond_to do |format|
        ...
        format.xls do
          @no_email = true
          data = render_to_string(template: 'profiles/list_summaries', layout: 'excel')
          send_data(data,
  ...
  def update
    ...
    mark_investigator_abstracts_as_reviewed(@investigator) if params[:commit] =~ /publication/i
    respond_to do |format|
  ...
  def edit_pubs
    ...
    @include_impact_factor = true
    investigator_id = 0
    investigator_id = @investigator.id unless @investigator.blank?
    respond_to do |format|
      format.html do
  ...
  def investigator_listing
    ...
    respond_to do |format|
      ...
      format.xls do
        @link_abstract_to_pubmed = true
        @abstracts = Abstract.display_all_investigator_data(@investigator.id)
        data = render_to_string(template: 'abstracts/investigator_listing', layout: 'excel')
        send_data(data,
  ...
  def render_abstract_listing(abstracts)
    ...
    respond_to do |format|
      ...
      format.xls do
        @link_abstract_to_pubmed = true
        data = render_to_string(template: 'profiles/abstracts_listing', layout: 'excel')
        send_data(data,
