...
class CytoscapeController < ApplicationController
  ...
  def simularity_wordle
    @title = 'Wordle for NO ONE BECAUSE THE ID IS INVALID'
    unless params[:id].blank?
      @investigators = Investigator.where('investigators.username in (:usernames)', { usernames: params[:id].split(',') })
  ...
  def d3_investigator_similarity_wordle_data
    words = []
    if params[:id]
      investigators = Investigator.where('investigators.username in (:usernames)', { usernames: params[:id].split(',') }).to_a
  ...
  def handle_data_params
    ...
    params[:include_publications] = '1' if params[:include_awards] == '0' && params[:include_studies] == '0'
