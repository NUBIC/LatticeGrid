...
class Log < ActiveRecord::Base
  ...
  scope :logins_for_investigator_ids, lambda { |*ids|
    joins(:investigator).where("activity = 'login' and logs.investigator_id IN (:ids) ",
      { :ids => ids.first })
  }
  scope :logins_after, lambda { |*ids|
    includes(:investigator)
      .where("logs.activity = 'login' and logs.created_at > :after_date",
        { :after_date => ( ids.first || '01-JAN-2000' ) })
  }
  scope :approved_profiles_after,  lambda { |*ids|
    where("logs.activity LIKE '%%update%%' and logs.params LIKE '%%Approve profile%%' and logs.created_at > :after_date",
      { :after_date => ( ids.first || '01-JAN-2000' ) })
  }
  scope :approved_publications_after, lambda { |*ids|
    where("logs.activity LIKE '%%update%%' and logs.params LIKE '%%Approve publication%%' and logs.created_at > :after_date",
      { :after_date => ( ids.first || '01-JAN-2000' ) })
  }
