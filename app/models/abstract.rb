...
class Abstract < ActiveRecord::Base
  ...
  scope :abstracts_last_five_years, where('abstracts.publication_date >= :start_date', { :start_date => 5.years.ago })

  scope :abstracts_by_date, lambda { |*dates|
    where('abstracts.publication_date between :start_date and :end_date', { :start_date => dates.first, :end_date => dates.last })
  }

  scope :ccsg_abstracts_by_date, lambda { |*dates|
    where('is_cancer = true and (abstracts.publication_date between :start_date and :end_date)', {:start_date => dates.first, :end_date => dates.last })
  }

  scope :with_impact_factor, lambda { |factor|
    joins(:journals).where('journals.impact_factor >= :impact_factor', { :impact_factor => factor })
  }
  ...
  scope :recently_changed, where('abstracts.updated_at >= :recent_date', { :recent_date => 30.days.ago }).order("abstracts.updated_at DESC")
  ...
  scope :by_investigator_ids, lambda { |*ids|
    joins(:investigator_abstracts).where('investigator_abstracts.investigator_id IN (:investigator_ids) AND investigator_abstracts.is_valid = true', { :investigator_ids => ids.first })
  }
  ...
  def self.full_author_not_has_full
    unscoped do
      abs = where("abstracts.full_authors is not null and not abstracts.full_authors = ''").all
      ary = []
