...
class Journal < ActiveRecord::Base
  ...
  def self.match_by_abbrev
    select('DISTINCT journals.id, journals.score_year, journals.impact_factor, journals.journal_abbreviation,
                     journals.issn, journals.total_cites, journals.impact_factor_five_year, journals.immediacy_index,
                     journals.total_articles, journals.eigenfactor_score, journals.article_influence_score, abstracts.issn as pubmed_issn')
  ...
  def self.preferred_high_impact_issns
    select('issn').where('include_as_high_impact = true').to_a
  end

  def self.high_impact(impact = 12.0)
    # order should be one of impact_factor DESC, count_all DESC, journals.journal_abbreviation
    select('id, score_year, impact_factor, journal_abbreviation, issn, total_cites, impact_factor_five_year,
           immediacy_index, total_articles, eigenfactor_score, article_influence_score')
  ...
  def self.journal_publications(years, sortby)
    ...
    conditions = "AND #{sortby.sub(/desc/i, '')} > 0.001" if (sortby =~ /desc/i) && (sortby !~ /count_all/i)

    select('count(*) as count_all, journals.id, journals.score_year, journals.impact_factor, journals.journal_abbreviation, journals.issn,
           journals.total_cites, journals.impact_factor_five_year, journals.immediacy_index, journals.total_articles,
           journals.eigenfactor_score, journals.article_influence_score')
      .joins(:abstracts)
      .where("abstracts.year IN (:years) AND journals.impact_factor > 0.001 #{conditions}", years: yearstring(years))
      .group('journals.id, journals.score_year, journals.impact_factor, journals.journal_abbreviation, journals.issn, journals.total_cites,
             journals.impact_factor_five_year, journals.immediacy_index, journals.total_articles, journals.eigenfactor_score, journals.article_influence_score')
      .order(sortby.to_s)
      .to_a
  end
  ...
  def self.journal_to_array(journals)
    journals.map { |x| x.journal_abbreviation.downcase }.sort.uniq.compact
  end
