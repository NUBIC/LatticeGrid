...
module SparklinesHelper
  ...
  def sparkline_barchart_setup(options = {})
    ...
    out = "<script type='text/javascript'>
      jQuery.noConflict();
      jQuery(function() {
          jQuery('.inlinebarchart').sparkline('html', #{sparkline_options(options)});
        });
      </script>"
    out
  end
  ...
  def js_sparkline_barchart(id, data, options = {})
    ...
    options['barWidth'] ||= 1
    out = "<script type='text/javascript'>
      jQuery('.#{id}').sparkline([#{data}], #{sparkline_options(options)});
     </script>"
    out
  end
  ...
  def jquery_sparkline_barchart(id, data, options = {})
    options['barSpacing'] ||= 1
    options['barWidth'] ||= 1
    out = "jQuery('.#{id}').sparkline([#{data}], #{sparkline_options(options)});"
    out
  end

  def sparkline_options(options)
    "{ type: 'bar', barColor: 'darkgrey', zeroColor: 'red', barWidth: #{options['barWidth']}, barSpacing: #{options['barSpacing']} }"
  end
  private :sparkline_options
end
