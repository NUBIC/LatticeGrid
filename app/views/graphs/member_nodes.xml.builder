...
xml.chart(:caption => @heading,
          :lineThickness=>'1', :showValues=>'1', :showFormBtn=>'0', :xAxisMinValue=>'0', :xAxisMaxValue=>'100',
          :yAxisMinValue=>'0', :yAxisMaxValue=>'100', :is3D=>'1', :animation=>'1', :palette=>'2', :formAction=>'/graphs/list',
          :formMethod=>'POST',  :enableLink=>'1' ) do
  xml.dataSet(:seriesName => "Investigator") do
    cnt=0
    investigator = @investigator
    xml.set(:x=>50, :y=>50, :width=>'120', :height=>'40',
            :name=>investigator.first_name+' '+investigator.last_name+"\n"+investigator.abstracts.length.to_s+' publications',
            :id=>investigator.id,
            :link => show_investigator_url(:id=>investigator.username, :page=>1),
            :color => '99CCFF',
