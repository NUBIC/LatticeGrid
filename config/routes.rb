...
#  map.connect 'abstracts/admin', :controller => 'abstracts', :action => 'ccsg', :conditions => { :method => :get } #legacy url. delete after 12/1/09
  ...
  map.resources :cytoscape, :only => [:index], :collection => { :jit => :get}, :member => {:investigators => :get, :investigator => :get}
  map.resources :mesh, :only => [:index], :member => {:search => :get, :investigators => :get, :investigator => :get}
...
# manually added rjs routes to enforce .js format
  ...
  map.resources :abstracts, :only => [:index, :show], :collection => { :search => [:get, :post], :ccsg => :get, :tag_cloud => :get, :current => :get, :add_pubmed_ids => [:get, :post], :update_pubmed_id => [:get, :post], :add_abstracts => :get },
    :member => {:set_deleted_date => [:get,:post], :set_investigator_abstract_end_date => [:get,:post],  :full_year_list => :get, :year_list => :get, :journal_list => :get, :endnote => :get, :full_tagged_abstracts => :get, :tagged_abstracts => [:get, :post] }
  map.resources :graphs, :only => [:none], :member => {:show_member => :get, :show_org => :get}
  map.resources :graphviz, :only => [:none], :member => {:show_member => :get, :show_member_mesh => :get, :show_mesh => :get, :show_org_mesh => :get, :show_org => :get}
